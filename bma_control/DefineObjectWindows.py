...
class DefineObjectWindow(ModalWindow):
    ...
    def __init__(self, handle_create_method, entry_label, parent, max_length=None, **kwargs):
        ...
        self.confirmation_box = ConfirmationBox(self.destroy, handle_create_method, "Hinzufügen")
...
class DefineCircuitWindow(DefineObjectWindow):
    ...
    def __init__(self, create_circuit_callback, parent):
        super().__init__(handle_create_method=self.handle_create_circuit,
                         entry_label="Nummer der Meldergruppe:",
                         parent=parent,
                         max_length=5)
        self.set_title("Meldergruppe hinzufügen")
        self.create_circuit_callback = create_circuit_callback

    def handle_create_circuit(self):
        try:
            ...
            return
        try:
            self.create_circuit_callback(circuit_number)
...
class DefineDetectorWindow(DefineObjectWindow):
    ...
    def __init__(self, circuit_number, create_detector_callback, parent):
        super().__init__(handle_create_method=self.handle_create_detector,
                         entry_label="Nummer des Melders:",
                         parent=parent,
                         max_length=2)
        self.set_title(f"Melder zu Meldergruppe {circuit_number} hinzufügen")
        self.create_detector_callback = create_detector_callback
    ...
    def handle_create_detector(self):
        ...
        try:
            ...
            self.create_detector_callback(self.circuit_number, detector_number, description)
        except ValueError as e:
            self.warning_label.set_markup(f"<span foreground='red'>{e}</span>")
