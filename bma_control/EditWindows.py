...
class EditWindow(ModalWindow):
    ...
    def __init__(self, edit_callback, parent, title, default_text: str="", max_length: int | None=None, description_box_label: str="Beschreibung:", **kwargs):
        ...
        self.confirmation_box = ConfirmationBox(self.destroy, edit_callback)
        ...
        self.set_default_widget(self.confirmation_box.confirm_button)
...
class EditDetectorWindow(EditWindow):
    ...
    def __init__(self, circuit_number, detector_number, current_description, edit_detector_callback, parent, **kwargs):
        ...
        super().__init__(self.handle_edit,
                         parent,
                         title,
                         current_description,
                         max_length=20,
                         **kwargs)
        self.edit_detector_callback = edit_detector_callback

    def handle_edit(self):
        description = self.get_description()
        try:
            self.edit_detector_callback(self.circuit_number, self.detector_number, description)
            self.destroy()
...
class EditBuildingWindow(EditWindow):
    ...
    def __init__(self, current_description, edit_building_callback, parent, **kwargs):
        ...
        super().__init__(self.handle_edit,
                         ...
                         **kwargs)
        self.edit_building_callback = edit_building_callback
    ...
    def handle_edit(self):
        description = self.get_description()
        try:
            self.edit_building_callback(description)
            self.destroy()
        except ValueError as e:
            self.warning_label.set_markup(f"<span foreground='red'>{e}</span>")
...
class CodeInputWindow(EditWindow):
    ...
    def handle_edit(self):
        ...
        if self.confirm_callback(self.unlock_action, code):
            self.destroy()
        else:
...
class EditCommitMessageWindow(ModalWindow):
    ...
    def __init__(self, finish_callback, file_type, parent, **kwargs):
        ...
        self.main_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL,
                                ...
                                spacing=10)
        self.set_child(self.main_box)
        self.commit_callback = finish_callback
        ...
        # Buttons to cancel or confirm
        self.confirmation_box = ConfirmationBox(self.destroy, self.handle_edit, "Bestätigen")
        self.main_box.append(self.confirmation_box)
    ...
    def handle_edit(self):
        ...
        self.commit_callback(message, self.file_type)
        self.destroy()